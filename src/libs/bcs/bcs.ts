...
import type { SuiObjectRef as SuiObjectRefType } from '@mysten/sui.js/client';
import { normalizeSuiAddress, SUI_ADDRESS_LENGTH } from '@mysten/sui.js/utils';

import { TypeTag, StructTag, TypeTagSerializer } from './typeTagSerializer';
...
const Address = bcs.bytes(SUI_ADDRESS_LENGTH).transform({
  input: (val: string | Uint8Array) =>
    typeof val === 'string' ? fromHEX(val) : val,
...
const ProgrammableMoveCall = bcs
  ...
  .transform({
    ...
    input: (data: any) => {
      ...
      const type_arguments = data.typeArguments.map((tag: any) =>
...
bcsRegistry.registerBcsType('utf8string', () =>
  bcs.string({ name: 'utf8string' })
);
